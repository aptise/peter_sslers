...
openssl genrsa -out selfsigned_1-server.key 2048
...
openssl genrsa -out selfsigned_2-server.key 2048
...
openssl genrsa -out selfsigned_3-server.key 2048
...
openssl genrsa -out selfsigned_4-server.key 2048
...
openssl genrsa -out selfsigned_5-server.key 2048
...
openssl genrsa -des3 -out selfsigned_1-server.key 2048
...
The PebbleCerts were ORIGINALLY generated by PeterSslers against a Pebble instance

The certs in the privkey1 folder can be replaced as-needed with manually signed ones using the 
same minica root that Pebble serves content under. The root pebble signs with changes on each invocation.

    cd privkey1
    python regenerate.py generate_csrs generate_certs
