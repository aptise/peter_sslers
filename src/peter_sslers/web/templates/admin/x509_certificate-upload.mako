...
<%block name="content_main">
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <%! show_text = False %>
            <form
                action="${admin_prefix}/x509-certificate/upload"
                method="POST"
        ...
        <div class="col-sm-6">
            ...
            <p><code>curl --form "private_key_file_pem=@privkey1.pem" --form "certificate_file=@cert1.pem" --form "chain_file=@chain1.pem" ${request.admin_url}/x509-certificate/upload</code></p>

            <p>But instead of that, post to <code>upload.json</code>, which will give you a json parcel in return</p>

            <p><code>curl --form "private_key_file_pem=@privkey1.pem" --form "certificate_file=@cert1.pem" --form "chain_file=@chain1.pem" ${request.admin_url}/x509-certificate/upload.json</code></p>
