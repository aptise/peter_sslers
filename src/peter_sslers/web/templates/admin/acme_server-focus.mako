    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <table class="table table-striped table-condensed">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        % if not AcmeServer.is_unlimited_pending_authz:
                            <form action="${admin_prefix}/acme-server/${AcmeServer.id}/mark" method="POST" style="display:inline;" id="form-mark-is_unlimited_pending_authz">
                ...
                <tr>
                    <th>is_retry_challenges</th>
                    <td>
                        <code>${AcmeServer.is_retry_challenges}</code>
                        % if not AcmeServer.is_retry_challenges:
                            <form action="${admin_prefix}/acme-server/${AcmeServer.id}/mark" method="POST" style="display:inline;" id="form-mark-is_retry_challenges">
                                <input type="hidden" name="action" value="is_retry_challenges-true"/>
                                <button class="btn btn-xs btn-primary" type="submit">
                                    <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                                    set
                                </button>
                        ...
                        % else:
                            <form action="${admin_prefix}/acme-server/${AcmeServer.id}/mark" method="POST" style="display:inline;" id="form-mark-is_retry_challenges">
                                <input type="hidden" name="action" value="is_retry_challenges-false"/>
                                <button class="btn btn-xs btn-danger" type="submit">
                                    <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                                    unset
                                </button>
