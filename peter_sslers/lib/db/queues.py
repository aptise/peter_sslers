...
def queue_domains__process(
    ...
    * if there are more than 100, should we process them, or return that info in json?
    ...
    try:
        ...
        (dbUniqueFqdnSet,
         _is_created,
         ) = lib.db.getcreate.getcreate__SslUniqueFQDNSet__by_domainObjects(ctx, domainObjects)
...
def queue_renewals__process(ctx):
    ...
    if items_count:
        ...
        for dbQueueRenewal in items_paged:
            if (not dbQueueRenewal.server_certificate) or (not dbQueueRenewal.server_certificate.ssl_acme_account_key_id) or (not dbQueueRenewal.server_certificate.ssl_acme_account_key.is_active):
        ...
        for dbQueueRenewal in items_paged:
            if (not dbQueueRenewal.server_certificate) or (not dbQueueRenewal.server_certificate.ssl_private_key_id__signed_by) or (not dbQueueRenewal.server_certificate.private_key.is_active):
