...
class ViewAdmin(Handler):
    ...
    def rewnewal_queue(self):
        items_count = lib_db.get__LetsencryptQueueRenewal__count(DBSession, show_all=False)
        (pager, offset) = self._paginate(items_count, url_template='/.well-known/admin/queue-renewals/{0}')
        items_paged = lib_db.get__LetsencryptQueueRenewal__paginated(DBSession, show_all=False, limit=items_per_page, offset=offset)
    ...
    def queue_renewal_all(self):
        items_count = lib_db.get__LetsencryptQueueRenewal__count(DBSession, show_all=True)
        (pager, offset) = self._paginate(items_count, url_template='/.well-known/admin/queue-renewals/all/{0}')
        items_paged = lib_db.get__LetsencryptQueueRenewal__paginated(DBSession, show_all=True, limit=items_per_page, offset=offset)
        return {'project': 'peter_sslers',
                'LetsencryptQueueRenewals_count': items_count,
                'LetsencryptQueueRenewals': items_paged,
                'sidenav_option': 'all',
    ...
    def _queue_renewal_focus(self):
        item = lib_db.get__LetsencryptQueueRenewal__by_id(DBSession, self.request.matchdict['id'])
        if not item:
